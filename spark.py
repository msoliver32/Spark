...
def parse(line):
    try:
        ...
        # Catch a invalid format line so return a black schema
...
if __name__ == "__main__":
    ...
    with open('./results/result_1.csv', 'w+') as csvFile:
        writer(csvFile).writerow([rddHosts.distinct().count()])
    ...
    rddTop_5_404 = rddDataParsed.map(lambda line: (line['request'], 1) if line['statusCode'] == "404" else ('', 1))
    ...
    with open('./results/result_3.csv', 'w+') as csvFile:
        writer(csvFile).writerows(rddTop_5_404.takeOrdered(5, key = lambda url: -url[1]))


    # 4. Total de erros 404  por dia.
    # Map our target data, if not 404 puts '' in timestamp key
    rddDays404 = rddDataParsed.map(lambda line: (line['timestamp'][:11], 1) if line['statusCode'] == "404" else ('', 1))
    # Removing invalid data
    ...
    rddSumBytes = rddDataParsed.map(lambda line: int(line['bytes']) if line['bytes'].isdigit() else 0)
